# Generate Verdi/nWave rc files containing top-level output ports for selected
# functional-simulation instances.
#
# Usage:
#   verdi -batch -play smart_run/impl/ptpx/script/extract_funcsim_output_rcs.tcl
#
# Optional overrides:
...
proc add_output_ports {win inst} {
  wvAddSignal -win $win -clear
  wvAddSignal -win $win -scope $inst -type output
}
...
foreach job $jobs {
  ...
  set out_rc [file join $out_dir "${block}_output.rc"]
  file mkdir $out_dir

  puts "INFO: extracting output ports for $inst"
  add_output_ports $wave_win $inst
