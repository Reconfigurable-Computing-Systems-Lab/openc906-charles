# Generate Verdi/nWave rc files containing top-level input ports for selected
# functional-simulation instances.
#
# Usage:
#   verdi -batch -play smart_run/impl/ptpx/script/extract_funcsim_input_rcs.tcl
#
# Optional overrides:
...
proc add_input_ports {win inst} {
  wvAddSignal -win $win -clear
  wvAddSignal -win $win -scope $inst -type input
}
...
foreach job $jobs {
  ...
  set out_rc [file join $out_dir "${block}_input.rc"]
  file mkdir $out_dir

  puts "INFO: extracting input ports for $inst"
  add_input_ports $wave_win $inst
